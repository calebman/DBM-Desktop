                                    ...
                                    <td v-if="cellMergeInit(rowIndex,col.field,item,true)"
                                        ...
                                        <span v-if="cellMergeContentType(rowIndex,col.field,item).isComponent">
                                            <component :rowData="item" :field="col.field ? col.field : ''"
                                                       ...
                                                       :pagingIndex="pagingIndex"></component>
                                        </span>
                            ...
                            <td v-if="cellMergeInit(rowIndex,col.field,item,false)"
                                ...
                                <span v-if="cellMergeContentType(rowIndex,col.field,item).isComponent">
                                    <component :rowData="item" :field="col.field ? col.field : ''" :index="rowIndex"
                                               :is="cellMerge(rowIndex,item,col.field).componentName"
                                               @on-custom-comp="customCompFunc"
                                               :cellClickData="cellClickData" :pagingIndex="pagingIndex"></component>
                                </span>
                                ...
                                <span v-if="typeof col.componentName ==='string' && col.componentName.length > 0">
                                    <component :rowData="item" :field="col.field ? col.field : ''" :index="rowIndex"
                                               :is="col.componentName" @on-custom-comp="customCompFunc"
                                               :cellClickData="cellClickData" :pagingIndex="pagingIndex"></component>
                                </span>
                                    <span v-else-if="typeof col.formatter==='function'"
...
<script>
    ...
    export default {
        ...
        components: {filterHeader, tableEmpty, loading, VCheckboxGroup, VCheckbox},
        data() {
            return {
                // 本地列表数据
                internalTableData: [],
...
//                },timer)
            ...
            onFilterChange(filterOpt){
                ...
                for(let index in filterParam){
                    ...
                    if(param.columnName == filterOpt.columnName){
                        findIndex = index;
                        break;
                    }
                }
                if(findIndex < 0)
                    filterParam.push(filterOpt);
                else filterParam.splice(findIndex,1,filterOpt);
                this.$emit('on-filter-change', filterParam);
            },
            onFilterDelete(columnName){
                let filterParam = this.filterParam;
                let findIndex = -1;
                for(let index in filterParam){
                    let param = filterParam[index];
