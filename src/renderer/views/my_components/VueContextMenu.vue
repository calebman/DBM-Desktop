<template>
  <span class="vue-contextmenu-listWrapper" :class="'vue-contextmenuName-' + contextMenuData.menuName">
    <li v-for="item in contextMenuData.menulists" class="context-menu-list">
      <Button type="text" size="large" :icon="item.iconName" @click.stop="fnHandler(item)">{{item.btnName}}</Button>
    </li>
  </span>
</template>
<script>
...
<style>
    ...
    .vue-contextmenu-listWrapper .context-menu-list button:hover {
        ...
        color: #ffffff;
        border-radius: 4px;
        background: -webkit-linear-gradient(bottom, #57a3f3, #7bbef3); /* Safari 5.1 - 6.0 */
        background: -o-linear-gradient(bottom, #57a3f3, #7bbef3); /* Opera 11.1 - 12.0 */
        background: -moz-linear-gradient(bottom, #57a3f3, #7bbef3); /* Firefox 3.6 - 15 */
        background: linear-gradient(to bottom, #57a3f3, #7bbef3);
    }
</style>
