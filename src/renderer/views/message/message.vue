...
export default {
    ...
    computed: {
        unreadMesList:{
            get(){
                return this.$store.state.msgs[0];
            },
            set(val){
                this.$store.state.msgs.splice(0,1,val)
            }
        },
        hasreadMesList:{
            get(){
                return this.$store.state.msgs[1];
            },
            set(val){
                this.$store.state.msgs.splice(1,1,val)
            }
        },
        recyclebinList:{
            get(){
                return this.$store.state.msgs[2];
            },
            set(val){
                this.$store.state.msgs.splice(2,1,val)
            }
        },
        unreadCount(){
            return this.$store.state.msgs[0].length;
        },
        hasreadCount(){
            return this.$store.state.msgs[1].length;
        },
        recyclebinCount(){
            return this.$store.state.msgs[2].length;
        }
    },
    methods: {
        formatDate (time) {
        ...
        getContent (index) {
            ...
            switch (this.currentMessageType) {
                case 'unread': mesContent = this.unreadMesList[index].msg; break;
                case 'hasread': mesContent =this.hasreadMesList[index].msg; break;
                default: mesContent =this.recyclebinList[index].msg; break;
            }
