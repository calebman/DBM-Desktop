...
<script>
    ...
    import filesPreview from './datatable/components/files-preview.vue'
    ...
    export default {
        ...
        methods: {
            init () {
                ...
                this.userName = cookie.get('cookie-user');
                this.checkTag(this.$route.name);
            },
            toggleClick () {
                this.$store.commit('changeHideMenuText');
            },
            handleClickUserDropdown (name) {
                if (name === 'ownSpace') {
                   ...
                   cookie.remove('cookie-password');
                   cookie.remove('cookie-hasGreet');
                   cookie.remove('cookie-access');
        ...
        mounted () {
            ...
            if (!cookie.get('cookie-hasGreet')) {
                ...
                let userName = cookie.get('cookie-user');
                if (hour > 5 && hour < 6) {
                ...
                cookie.set('cookie-hasGreet', 1);
            }
        },
        created () {
            // 查找当前用户之前登录时设置的主题
            let name = cookie.get('cookie-user');
            if (localStorage.theme) {
            ...
            if (this.$store.state.theme !== 'b') {
                let stylesheetPath = 'static/' + this.$store.state.theme + '.css';
                let themeLink = document.querySelector('link[name="theme"]');
